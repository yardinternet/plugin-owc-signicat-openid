...
class OpenIDField extends GF_Field
{
	...
	public function get_field_input($form, $value = '', $entry = null)
	{
		...
		if ($this->idp->getSlug() === 'digid') {
			$overwrite = DigiDSession::isLoggedIn() && ! is_null(DigiDSession::getUserData());
		} elseif ($this->idp->getSlug() === 'eherkenning') {
			$overwrite = EherkenningSession::isLoggedIn() && ! is_null(EherkenningSession::getUserData());
		}

		if ($overwrite || ($this->openIDService->getUserInfo($this->idp) && ! $this->is_form_editor())) {
			return sprintf(
				"<div class='ginput_container ginput_container_openid'><input name='input_%d' id='%s' type='hidden' value='1'>%s</div>",
				$this->id,
	...
	{
		if ($this->session_user_data_is_active()) {
	...
	private function session_user_data_is_active(): bool
	{
